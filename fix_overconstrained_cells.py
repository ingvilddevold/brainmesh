...
app = typer.Typer(help="Refine overconstrained cells in dolfinx meshes and preserve all tags.")


def read_all_tags(
    mesh_path: Path, cell_tag_names: list[str], facet_tag_names: list[str]
) -> tuple[dolfinx.mesh.Mesh, dict[str, dolfinx.mesh.MeshTags], dict[str, dolfinx.mesh.MeshTags]]:
    ...
    with XDMFFile(MPI.COMM_WORLD, mesh_path, "r") as xdmf:
        ...
        for name in cell_tag_names:
            try:
                ...
                if MPI.COMM_WORLD.rank == 0:
                    print(f"Warning: Cell tag '{name}' not found in the input mesh. Skipping.")
        ...
        for name in facet_tag_names:
            try:
                ...
                if MPI.COMM_WORLD.rank == 0:
                    print(f"Warning: Facet tag '{name}' not found in the input mesh. Skipping.")
...
def fix_overconstrained_cells(
...
) -> tuple[int, dolfinx.mesh.Mesh, dict[str, dolfinx.mesh.MeshTags], dict[str, dolfinx.mesh.MeshTags]]:
    ...
    # Data structures to handle arbitrary numbers of tags dynamically
    ...
    # O(1) facet lookups instead of slow np.flatnonzero loops
    facet_tag_lookups = {
        k: dict(zip(ft.indices, ft.values)) for k, ft in facet_tags.items()
    }

    for i in tqdm(range(num_cells), desc="Checking cells"):
        ...
        if len(exterior_vertices) == 4:
            ...
            for k in facet_tags.keys():
                if interior_facets[0] in facet_tag_lookups[k]:
                    ...
                    for _ in range(3):
                        new_facet_marker_arrays[k].append(facet_tag_lookups[k][interior_facets[0]])
    ...
    # Initialize new mesh (USING 'e' INSTEAD OF 'domain' TO AVOID THE VALUE/TYPE ERRORS)
    ...
    for k, ft in facet_tags.items():
        ...
        new_facet_values_array = ft.values[facets_to_keep]
        ...
        if len(new_marked_facets[k]) > 0:
            marked_facets_array = np.array(new_marked_facets[k], dtype=np.int64).reshape(-1, 3)
        ...
        new_ft = dolfinx.mesh.meshtags_from_entities(
            new_mesh, new_mesh.topology.dim - 1, adj, local_values.astype(np.int32, copy=False)
        )
        new_ft.name = k
...
def fix(
    ...
    outfile: Path = typer.Option(None, "-o", "--output", help="The output XDMF mesh file"),
    cell_tag_name1: str = typer.Option("subdomains", help="Name of the primary cell MeshTags"),
    cell_tag_name2: str = typer.Option("subdomains_ftetwild", help="Name of the secondary cell MeshTags"),
    ...
    mesh, cell_tags, facet_tags = read_all_tags(infile, expected_cell_tags, expected_facet_tags)

    print(f"Number of cells before fix: {mesh.topology.index_map(mesh.topology.dim).size_global}")
