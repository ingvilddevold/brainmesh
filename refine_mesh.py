...
app = typer.Typer(help="Refine an input XDMF mesh locally around SSAS/Aqueduct and update FEniCSx tags.")
...
def refine_mesh(
    ...
    for i in range(num_refinements):
        ...
        v3_v4_cells = ftetwild_markers.indices[np.isin(ftetwild_markers.values, [V4_FTW, V3_FTW])]
        ...
        if "boundaries" in facet_tags:
            ...
            if mesh.comm.rank == 0:
                print("Warning: 'boundaries' tag not found. Cannot explicitly target SSAS facets.")

        # Combine localized targets
        cells_to_refine = np.unique(np.hstack([v3_v4_cells, ssas_cells])).astype(np.int32)
        ...
        for k, ft in facet_tags.items():
            facet_tags[k] = dolfinx.mesh.transfer_meshtag(ft, mesh, parent_cell, parent_facet)
            facet_tags[k].name = f_names[k]  # Restore names

        # Print the updated cell count
        if mesh.comm.rank == 0:
            print(f"Refinement {i + 1}/{num_refinements}: cell count = {mesh.topology.index_map(tdim).size_global}")
...
@app.command()
def refine(
    input_mesh: Path = typer.Argument(..., help="Path to the input XDMF mesh containing tags."),
    ...
    mesh_tags_name: str = typer.Option(
        "subdomains", help="Name of the main region marker MeshTags inside the input XDMF file (used to decide refinement areas)."
    ),
    ...
    mesh, cell_tags, facet_tags = read_all_tags(input_mesh, expected_cell_tags, expected_facet_tags)
    
    if "subdomains_ftetwild" not in cell_tags:
        raise ValueError("Required cell tag 'subdomains_ftetwild' was not found in the input mesh.")

    # Apply highly localized refinement and transfer all loaded tags
    mesh, cell_tags, facet_tags = refine_mesh(
        mesh, cell_tags, facet_tags, primary_region_name=mesh_tags_name, num_refinements=refinement_steps
    )
